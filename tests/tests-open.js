...
export default () => {
  ...
  group('Corpus Christi Ouro Preto', function () {
    ...
    // Remoção do feriado estadual funciona como esperado
    let del1 = http.del(`${BASE_URL}/feriados/3146107/corpus-christi/`);
    check(del1, {
      'remover o feriado municipal Corpus Christi retorna status 204':
      (r) => r.status === 204,
    });
  });
  group('Carnaval na Bahia', function () {
    // Cadastro de feriado móvel estadual
    let register = http.put(`${BASE_URL}/feriados/29/carnaval/`, { headers: HEADERS });
    check(register, {
      'cadastro retorna status 200 ou 201':
      (r) => [200, 201].includes(r.status),
    });

    // Consulta feriado móvel estadual
    let query1 = http.get(`${BASE_URL}/feriados/29/2020-02-25/`);
    check(query1, {
      'consulta deve retornar 200': (r) => r.status === 200
    });
    check(query1.json(),
      {
        'é Carnaval na Bahia em 2020':
        (obj) => obj.name === 'Carnaval',
      }
    );

    // Consulta feriado móvel municipal
    let query2 = http.get(`${BASE_URL}/feriados/2927408/2021-02-16/`);
    check(query2, {
      'consulta deve retornar 200': (r) => r.status === 200
    });
    check(query2.json(),
      {
        'é Carnaval em Salvador em 2021':
        (obj) => obj.name === 'Carnaval',
      }
    );
    // Remoção do feriado municipal é proibido
    let del1 = http.del(`${BASE_URL}/feriados/2927408/carnaval/`);
    check(del1, {
      'tentar remover um feriado estadual no município retorna status 403':
      (r) => r.status === 403,
    });
    // Remoção do feriado estadual funciona como esperado
    let del2 = http.del(`${BASE_URL}/feriados/29/carnaval/`);
    check(del2, {
      'remover o feriado estadual Carnaval retorna status 204':
      (r) => r.status === 204,
    });
  });
}
